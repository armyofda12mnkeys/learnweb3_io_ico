...
export default function Home() {
  ...
  const getTokensToBeClaimed = async () => {
    try {
      ...
      let address;
      if( signer ){ //also 'as providers.JsonRpcSigner' above
  ...
  const mintCryptoDevToken = async (amount:BigNumber) => {
    try {
      ...
      const etherCost = utils.parseEther(".001"); 
      console.log('etherCost(in wei aka 10^18)', etherCost.toString());
      const value = amount.mul( etherCost) ;
      console.log('value', value.toNumber());
      
      //const value = utils.parseUnits( etherCostMult ); //  .mul(0.001);
      
      const tx = await tokenContract.mint(amount.toNumber(), {
